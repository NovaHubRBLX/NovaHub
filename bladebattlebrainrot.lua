...
TopBarLabel.Size = UDim2.new(1, -110, 1, 0)
TopBarLabel.Position = UDim2.new(0, 30, 0, 0)
TopBarLabel.BackgroundTransparency = 1
TopBarLabel.Text = "NOVA HUB | BLADE BATTLE FOR BRAINROT"
TopBarLabel.Font = Enum.Font.GothamBold
TopBarLabel.TextSize = 13
TopBarLabel.TextColor3 = Colors.TextMuted
...
local function addDropdownButton(page, text, order)
    local isOpen = false
    ...
    local optionsContainer = Instance.new("ScrollingFrame", container)
    optionsContainer.Position = UDim2.new(0, 0, 0, 45)
    optionsContainer.Size = UDim2.new(1, 0, 1, -55)
    ...
    Instance.new("UIListLayout", optionsContainer).Padding = UDim.new(0, 5)
    ...
    local function refreshItems()
        ...
        -- Corrected path: Look for 'model' (lowercase)
        local brainrotFolder = game.Workspace:FindFirstChild("BrainrotFolder")
        local modelFolder = brainrotFolder and brainrotFolder:FindFirstChild("model")
        
        if modelFolder then
            for _, rarityFolder in pairs(modelFolder:GetChildren()) do
                if rarityFolder:IsA("Folder") then
                    for _, item in pairs(rarityFolder:GetChildren()) do
                        -- Check if name contains "Brainrot_"
                        if string.find(item.Name, "Brainrot_") then
                            -- Support both Parts and Models
                            local targetPart = item:IsA("BasePart") and item or (item:IsA("Model") and item.PrimaryPart or nil)
                            
                            if targetPart then
                                local opt = Instance.new("TextButton", optionsContainer)
                                opt.Size = UDim2.new(1, -20, 0, 30)
                                opt.BackgroundColor3 = Colors.SidebarBg
                                opt.Text = item.Name .. " (" .. rarityFolder.Name .. ")"
                                opt.Font = Enum.Font.Gotham
                                opt.TextColor3 = Colors.TextMuted
                                opt.TextSize = 12
                                opt.ZIndex = 12
                                Instance.new("UICorner", opt).CornerRadius = UDim.new(0, 6)
                                
                                opt.MouseButton1Click:Connect(function()
                                    if Player.Character then Player.Character:PivotTo(targetPart.CFrame) end
                                end)
                            end
                        end
                    end
            ...
            warn("Could not find: Workspace -> BrainrotFolder -> model")
        end
    end

    -- Refresh loop (10 seconds)
    task.spawn(function()
        while true do
            refreshItems()
            task.wait(10)
    ...
    mainBtn.MouseButton1Click:Connect(function()
        isOpen = not isOpen
        TweenService:Create(container, TWEEN_MED, {Size = UDim2.new(1, 0, 0, isOpen and 200 or 42)}):Play()
    end)
end

local function StartAutoCollect(enabled)
    getgenv().AutoCollectRunning = enabled
    if enabled then
        task.spawn(function()
            while getgenv().AutoCollectRunning do
                for i = 1, 30 do
                    if not getgenv().AutoCollectRunning then break end
...
addSectionLabel(dashboardPage, "OVERVIEW", 1)
addStatCard(dashboardPage, "STATUS", "Online", 2)
addSectionLabel(dashboardPage, "ACTIONS", 3)
addToggle(dashboardPage, "Fly Mode", 5, false, function(state)
    setFly(state)
end)
addToggle(dashboardPage, "Noclip (Pass Walls)", 4, false, function(state)
    setNoclip(state)
end)

addSectionLabel(autoPage, "AUTOMATION", 1)
addToggle(autoPage, "Auto Collect Gold", 2, false, function(state)
    StartAutoCollect(state)
end)

addSectionLabel(teleportPage, "TELEPORTATION", 1)
local rarities = {"Common", "Divine", "AscendantCelestial", "AscendantEternal", "AscendantExclusive", "AscendantOG", "Celestial", "Epic", "Eternal", "Exclusive", "God", "Legendary", "Mythic", "Og", "Rare", "Secret"}

-- Use it in your dashboard setup:
addDropdownButton(teleportPage, "TP to Brainrot", 6)

addButton(teleportPage, "Teleport to Lobby", 5, function()
    ...
    if spawn and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        -- Teleports slightly above the spawn pad to prevent getting stuck
...
addWalkSpeedSlider(settingsPage, 2)

addSectionLabel(settingsPage, "PREFERENCES", 1)
addToggle(settingsPage, "Notifications", 2, true, function(state)
    print("Notifications:", state)
end)
addToggle(settingsPage, "Sound Effects", 3, true, function(state)
...
tabButtons["Teleport"].button.MouseButton1Click:Connect(function() switchTab("Teleport") end)
...
MinimizeBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        expandedSize = Main.Size -- remember current (possibly resized) size before collapsing
        TweenService:Create(Main, TWEEN_MED, {Size = UDim2.new(0, Main.Size.X.Offset, 0, 40)}):Play()
        MinimizeBtn.Text = "▢"
        ResizeGrip.Visible = false
    else
        TweenService:Create(Main, TWEEN_MED, {Size = expandedSize}):Play()
        MinimizeBtn.Text = "—"
